...
\begin{table}[H]
	...
	\begin{tabular}{c c c c c c} 
		\hline
		\textbf{Metryka} & \textbf{Skuteczność} & \textbf{Rodzaj ekstrakcji} &  \textbf{Liczba k sąsiadów} & \textbf{Kategoria} & \textbf{Zbiór treningowy}\\ [0.5ex]
		\hline
		\hline 
		Euklidesowa & 83.3\% & IDF & 7 & Places & 60\% \\
		Uliczna & 83.3\% & IF & 7 & Places & 60\% \\
		Czebyszewa & 80.3\% & TF & 2-20 & Places & 60\% \\
		\hline
	\end{tabular}
	\caption{Tabela przedstawiająca najwyżej osiągnięte wyniki przez metrykę Euklidesową, uliczną oraz Czebyszewa z pierwszego eksperymentu (4.1)}
\end{table}
...
\subsection{Wpływ podziału tekstów na zbiory treningowe i testowe na klasyfikację}
W przeważającej większości najwyższe wyniki osiągane były przy 80% zbioru treningowego. Tylko w jednym przypadku użycie 40% zbioru treningowego pozwoliło osiągnać najwyższą skutecznośći (pierwszy sposób ekstrakcji, kategoria topics). Zazwyczaj jednak ten dobór procentowy okazywał się być najsłabszym. 
\newline
...
Podczas klasyfikacji dla kategorii authors wyraźnie pozytywny wpływ na skuteczność ma cecha odpowiadająca za liczbę unikalnych słów. Bez niej skutecznośc spadła z 71% na 47%. Podobnie jak w przypadku kategorii topics, cechy sprawdzające liczbę słów oraz liczbę krótkich słów mogły osłabiać nasze wyniki. 

	
\section{Wnioski}
\begin{itemize}
	\item Cecha odpowiedzialna za liczbę słów w dokumencie nie sprawdza się w przypadku klasyfikacji tekstów.
	\item Wektor cech nie powinien składać się z pojedynczej cechy, gdyż wtedy osiągana jest niska skuteczność.
	\item Metryka Czebyszewa nie powinna być wykorzystywana w klasyfikacji tekstów, gdyż osiąga bardzo słabe wyniki.
	\item Liczba k sąsiadów ma spory wpływ na skuteczność klasyfikacji.
\end{itemize}
